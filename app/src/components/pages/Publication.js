import React from "react";

const Publication = () => {
  return (
    <>
      <div className="container">
        <section id="publication" style={{ height: "100vh" }}>
          <div className="h4 text-center font-weight-bold">Publication</div>
          <hr className="w-25 m-auto" />
          <br />
          <br />
          <br />
          <div className="d-flex flex-row pt-3" style={{padding:'5px',borderRadius:'15px', borderColor:'blue', border:'1px solid blue'}}>
            <div className="col">
              <h4>
                Automated GDPR Contract Compliance Verification Using Knowledge
                Graphs
              </h4>
              <h6>
                Amar Tauqeer, Anelia Kurteva, Tek Raj Chhetri, Albin Ahmeti,
                Anna Fensel.
              </h6>
              <div className="d-flex flex-row w-25 pb-3">
                <div className="col">
                  <a href="https://www.mdpi.com/2078-2489/13/10/447/pdf" target="_blank">
                    PDF
                  </a>
                </div>
                <div className="col">
                  <a href="https://doi.org/10.3390/info13100447" target="_blank">
                    DOI
                  </a>
                </div>
              </div>
            </div>
          </div>
          <br />
          <div className="d-flex flex-row pt-3" style={{padding:'5px',borderRadius:'15px', borderColor:'blue', border:'1px solid blue'}}>
            <div className="col">
              <h4>
              Towards Knowledge Graphs Validation Through Weighted Knowledge Sources

              </h4>
              <h6>
              E Huaman, A Tauqeer, A Fensel Iberoamerican Knowledge Graphs and Semantic Web Conference, 47-60.
              </h6>
              <div className="d-flex flex-row w-25 pb-3">
                <div className="col">
                  <a href="https://arxiv.org/pdf/2104.12622" target="_blank">
                    PDF
                  </a>
                </div>
                <div className="col">
                  <a href="https://link.springer.com/chapter/10.1007/978-3-030-91305-2_4" target="_blank">
                    DOI
                  </a>
                </div>
              </div>
            </div>
          </div>
          <br />
          <div className="d-flex flex-row pt-3" style={{padding:'5px',borderRadius:'15px', borderColor:'blue', border:'1px solid blue'}}>
            <div className="col">
              <h4>
              Data sharing in smashHit: Making consent and contracts

              </h4>
              <h6>
              Anelia Kurteva, Amar Tauqeer, Anna Fensel.
              </h6>
              <div className="d-flex flex-row w-25 pb-3">
                <div className="col">
                  <a href="https://www.trusts-data.eu/wp-content/uploads/2022/06/04_smashHit-Fensel_220603_WSSemanticInteroperability.pdf" target="_blank">
                    PDF
                  </a>
                </div>
                {/* <div className="col">
                  <a href="" target="_blank">
                    DOI
                  </a>
                </div> */}
              </div>
            </div>
          </div>
        </section>
      </div>
    </>
  );
};

export default Publication;
